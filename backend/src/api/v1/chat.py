...
class MockAgentService:
    """Mock agent service for testing when API keys are not available or rate limits are hit."""
    ...
    def generate_response(self, question: str, retrieved_docs):
        ...
        # Strictly follow the requirements: answer only from book content
        if not retrieved_docs:
            answer = "This information is not available in the book."
        else:
            # Check if the question can be answered from the context
            ...
            for doc in retrieved_docs:
                if question_lower in doc.content.lower():
                    context_found = True
                    relevant_content = doc.content[:200]  # Take first 200 chars as sample
                    break

            if context_found:
                answer = f"Based on the book content: I found information related to your question '{question}' in the provided documents."
            else:
                answer = "This information is not available in the book."
...
def get_agent_service():
    ...
    try:
        ...
        logger.warning(f"Agent service not available due to error: {e}")
        logger.info("Using mock agent service as fallback")
